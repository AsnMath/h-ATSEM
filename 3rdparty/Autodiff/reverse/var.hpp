...
namespace autodiff
{
    ...
    namespace reverse
    {
        ...
        namespace detail
        {
            ...
            namespace traits
            {
                ...
                template <typename T>
                struct VariableValueType<Variable<T>>
                {
            ...
            template <typename T>
            ExprPtr<T> constant(const T &val) { return std::make_shared<ConstantExpr<T>>(val); }
        ...
        return op(l, r); });
            ...
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> operator+(const ExprPtr<T> &r) { return r; }
            template <typename T>
            ExprPtr<T> operator-(const ExprPtr<T> &r) { return std::make_shared<NegativeExpr<T>>(-r->val, r); }

            template <typename T>
            ExprPtr<T> operator+(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<AddExpr<T>>(l->val + r->val, l, r); }
            template <typename T>
            ExprPtr<T> operator-(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<SubExpr<T>>(l->val - r->val, l, r); }
            template <typename T>
            ExprPtr<T> operator*(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<MulExpr<T>>(l->val * r->val, l, r); }
            template <typename T>
            ExprPtr<T> operator/(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<DivExpr<T>>(l->val / r->val, l, r); }

            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator+(const U &l, const ExprPtr<T> &r) { return constant<T>(l) + r; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator-(const U &l, const ExprPtr<T> &r) { return constant<T>(l) - r; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator*(const U &l, const ExprPtr<T> &r) { return constant<T>(l) * r; }
            ...
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator+(const ExprPtr<T> &l, const U &r) { return l + constant<T>(r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator-(const ExprPtr<T> &l, const U &r) { return l - constant<T>(r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator*(const ExprPtr<T> &l, const U &r) { return l * constant<T>(r); }
            ...
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sin(const ExprPtr<T> &x) { return std::make_shared<SinExpr<T>>(sin(x->val), x); }
            template <typename T>
            ExprPtr<T> cos(const ExprPtr<T> &x) { return std::make_shared<CosExpr<T>>(cos(x->val), x); }
            template <typename T>
            ExprPtr<T> tan(const ExprPtr<T> &x) { return std::make_shared<TanExpr<T>>(tan(x->val), x); }
            template <typename T>
            ExprPtr<T> asin(const ExprPtr<T> &x) { return std::make_shared<ArcSinExpr<T>>(asin(x->val), x); }
            template <typename T>
            ExprPtr<T> acos(const ExprPtr<T> &x) { return std::make_shared<ArcCosExpr<T>>(acos(x->val), x); }
            template <typename T>
            ExprPtr<T> atan(const ExprPtr<T> &x) { return std::make_shared<ArcTanExpr<T>>(atan(x->val), x); }
            template <typename T>
            ExprPtr<T> atan2(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<ArcTan2Expr<T>>(atan2(l->val, r->val), l, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> atan2(const U &l, const ExprPtr<T> &r) { return std::make_shared<ArcTan2Expr<T>>(atan2(l, r->val), constant<T>(l), r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> atan2(const ExprPtr<T> &l, const U &r) { return std::make_shared<ArcTan2Expr<T>>(atan2(l->val, r), l, constant<T>(r)); }

            //------------------------------------------------------------------------------
            // HYPOT2 FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<Hypot2Expr<T>>(hypot(l->val, r->val), l, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const U &l, const ExprPtr<T> &r) { return std::make_shared<Hypot2Expr<T>>(hypot(l, r->val), constant<T>(l), r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const U &r) { return std::make_shared<Hypot2Expr<T>>(hypot(l->val, r), l, constant<T>(r)); }

            //------------------------------------------------------------------------------
            // HYPOT3 FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const ExprPtr<T> &c, const ExprPtr<T> &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l->val, c->val, r->val), l, c, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const ExprPtr<T> &c, const U &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l->val, c->val, r), l, c, constant<T>(r)); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const U &l, const ExprPtr<T> &c, const ExprPtr<T> &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l, c->val, r->val), constant<T>(l), c, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const U &c, const ExprPtr<T> &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l->val, c, r->val), l, constant<T>(c), r); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const ExprPtr<T> &l, const U &c, const V &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l->val, c, r), l, constant<T>(c), constant<T>(r)); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const U &l, const ExprPtr<T> &c, const V &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l, c->val, r), constant<T>(l), c, constant<T>(r)); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const V &l, const U &c, const ExprPtr<T> &r) { return std::make_shared<Hypot3Expr<T>>(hypot(l, c, r->val), constant<T>(l), constant<T>(c), r); }

            //------------------------------------------------------------------------------
            // HYPERBOLIC FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sinh(const ExprPtr<T> &x) { return std::make_shared<SinhExpr<T>>(sinh(x->val), x); }
            template <typename T>
            ExprPtr<T> cosh(const ExprPtr<T> &x) { return std::make_shared<CoshExpr<T>>(cosh(x->val), x); }
            template <typename T>
            ExprPtr<T> tanh(const ExprPtr<T> &x) { return std::make_shared<TanhExpr<T>>(tanh(x->val), x); }

            //------------------------------------------------------------------------------
            // EXPONENTIAL AND LOGARITHMIC FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> exp(const ExprPtr<T> &x) { return std::make_shared<ExpExpr<T>>(exp(x->val), x); }
            template <typename T>
            ExprPtr<T> log(const ExprPtr<T> &x) { return std::make_shared<LogExpr<T>>(log(x->val), x); }
            template <typename T>
            ExprPtr<T> log10(const ExprPtr<T> &x) { return std::make_shared<Log10Expr<T>>(log10(x->val), x); }

            //------------------------------------------------------------------------------
            // POWER FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sqrt(const ExprPtr<T> &x) { return std::make_shared<SqrtExpr<T>>(sqrt(x->val), x); }
            template <typename T>
            ExprPtr<T> pow(const ExprPtr<T> &l, const ExprPtr<T> &r) { return std::make_shared<PowExpr<T>>(pow(l->val, r->val), l, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> pow(const U &l, const ExprPtr<T> &r) { return std::make_shared<PowConstantLeftExpr<T>>(pow(l, r->val), constant<T>(l), r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> pow(const ExprPtr<T> &l, const U &r) { return std::make_shared<PowConstantRightExpr<T>>(pow(l->val, r), l, constant<T>(r)); }

            //------------------------------------------------------------------------------
            // OTHER FUNCTIONS
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> abs(const ExprPtr<T> &x) { return std::make_shared<AbsExpr<T>>(abs(x->val), x); }
            template <typename T>
            ExprPtr<T> abs2(const ExprPtr<T> &x) { return x * x; }
            template <typename T>
            ExprPtr<T> conj(const ExprPtr<T> &x) { return x; }
            template <typename T>
            ExprPtr<T> real(const ExprPtr<T> &x) { return x; }
            template <typename T>
            ExprPtr<T> imag(const ExprPtr<T> &) { return constant<T>(0.0); }
            template <typename T>
            ExprPtr<T> erf(const ExprPtr<T> &x) { return std::make_shared<ErfExpr<T>>(erf(x->val), x); }
            ...
            T expr_value(const T &t) { return t; }
            template <typename T>
            T expr_value(const ExprPtr<T> &t) { return t->val; }
            template <typename T>
            T expr_value(const Variable<T> &t) { return t.expr->val; }
            ...
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> coerce_expr(const U &u) { return constant<T>(u); }
            template <typename T>
            ExprPtr<T> coerce_expr(const ExprPtr<T> &t) { return t; }
            template <typename T>
            ExprPtr<T> coerce_expr(const Variable<T> &t) { return t.expr; }

            template <typename T, typename U>
            struct is_binary_expr : std::conditional_t<!(isArithmetic<T> && isArithmetic<U>) && is_expr_v<T> && is_expr_v<U>, std::true_type, std::false_type>
            {
            };
            template <typename T, typename U>
            ...
            template <typename T, typename U, Requires<is_binary_expr_v<T, U>> = true>
            auto min(const T &x, const U &y) { return condition(x < y, x, y); }
            template <typename T, typename U, Requires<is_binary_expr_v<T, U>> = true>
            auto max(const T &x, const U &y) { return condition(x > y, x, y); }
            template <typename T>
            ExprPtr<T> sgn(const ExprPtr<T> &x) { return condition(x < 0, -1.0, condition(x > 0, 1.0, 0.0)); }
            template <typename T>
            ExprPtr<T> sgn(const Variable<T> &x) { return condition(x.expr < 0, -1.0, condition(x.expr > 0, 1.0, 0.0)); }

            //------------------------------------------------------------------------------
            // ARITHMETIC OPERATORS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            const ExprPtr<T> &operator+(const Variable<T> &r) { return r.expr; }
            template <typename T>
            ExprPtr<T> operator-(const Variable<T> &r) { return -r.expr; }
            ...
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator+(const U &l, const Variable<T> &r) { return l + r.expr; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator-(const U &l, const Variable<T> &r) { return l - r.expr; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator*(const U &l, const Variable<T> &r) { return l * r.expr; }
            ...
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator+(const Variable<T> &l, const U &r) { return l.expr + r; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator-(const Variable<T> &l, const U &r) { return l.expr - r; }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> operator*(const Variable<T> &l, const U &r) { return l.expr * r; }
            ...
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sin(const Variable<T> &x) { return sin(x.expr); }
            template <typename T>
            ExprPtr<T> cos(const Variable<T> &x) { return cos(x.expr); }
            template <typename T>
            ExprPtr<T> tan(const Variable<T> &x) { return tan(x.expr); }
            template <typename T>
            ExprPtr<T> asin(const Variable<T> &x) { return asin(x.expr); }
            template <typename T>
            ExprPtr<T> acos(const Variable<T> &x) { return acos(x.expr); }
            template <typename T>
            ExprPtr<T> atan(const Variable<T> &x) { return atan(x.expr); }
            template <typename T>
            ExprPtr<T> atan2(const Variable<T> &l, const Variable<T> &r) { return atan2(l.expr, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> atan2(const U &l, const Variable<T> &r) { return atan2(l, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> atan2(const Variable<T> &l, const U &r) { return atan2(l.expr, r); }

            //------------------------------------------------------------------------------
            // HYPOT2 FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> hypot(const Variable<T> &l, const Variable<T> &r) { return hypot(l.expr, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const U &l, const Variable<T> &r) { return hypot(l, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const Variable<T> &l, const U &r) { return hypot(l.expr, r); }

            //------------------------------------------------------------------------------
            // HYPOT3 FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> hypot(const Variable<T> &l, const Variable<T> &c, const Variable<T> &r) { return hypot(l.expr, c.expr, r.expr); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const Variable<T> &l, const U &c, const V &r) { return hypot(l.expr, c, r); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const U &l, const Variable<T> &c, const V &r) { return hypot(l, c.expr, r); }
            template <typename T, typename U, typename V, Requires<isArithmetic<U> && isArithmetic<V>> = true>
            ExprPtr<T> hypot(const U &l, const V &c, const Variable<T> &r) { return hypot(l, c, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const Variable<T> &l, const Variable<T> &c, const U &r) { return hypot(l.expr, c.expr, r); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const U &l, const Variable<T> &c, const Variable<T> &r) { return hypot(l, c.expr, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> hypot(const Variable<T> &l, const U &c, const Variable<T> &r) { return hypot(l.expr, c, r.expr); }

            //------------------------------------------------------------------------------
            // HYPERBOLIC FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sinh(const Variable<T> &x) { return sinh(x.expr); }
            template <typename T>
            ExprPtr<T> cosh(const Variable<T> &x) { return cosh(x.expr); }
            template <typename T>
            ExprPtr<T> tanh(const Variable<T> &x) { return tanh(x.expr); }

            //------------------------------------------------------------------------------
            // EXPONENTIAL AND LOGARITHMIC FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> exp(const Variable<T> &x) { return exp(x.expr); }
            template <typename T>
            ExprPtr<T> log(const Variable<T> &x) { return log(x.expr); }
            template <typename T>
            ExprPtr<T> log10(const Variable<T> &x) { return log10(x.expr); }

            //------------------------------------------------------------------------------
            // POWER FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> sqrt(const Variable<T> &x) { return sqrt(x.expr); }
            template <typename T>
            ExprPtr<T> pow(const Variable<T> &l, const Variable<T> &r) { return pow(l.expr, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> pow(const U &l, const Variable<T> &r) { return pow(l, r.expr); }
            template <typename T, typename U, Requires<isArithmetic<U>> = true>
            ExprPtr<T> pow(const Variable<T> &l, const U &r) { return pow(l.expr, r); }

            //------------------------------------------------------------------------------
            // OTHER FUNCTIONS (DEFINED FOR ARGUMENTS OF TYPE Variable)
            //------------------------------------------------------------------------------
            template <typename T>
            ExprPtr<T> abs(const Variable<T> &x) { return abs(x.expr); }
            template <typename T>
            ExprPtr<T> abs2(const Variable<T> &x) { return abs2(x.expr); }
            template <typename T>
            ExprPtr<T> conj(const Variable<T> &x) { return conj(x.expr); }
            template <typename T>
            ExprPtr<T> real(const Variable<T> &x) { return real(x.expr); }
            template <typename T>
            ExprPtr<T> imag(const Variable<T> &x) { return imag(x.expr); }
            template <typename T>
            ExprPtr<T> erf(const Variable<T> &x) { return erf(x.expr); }
    ...
    inline reverse::detail::BooleanExpr boolref(const bool &v)
    {
        return reverse::detail::BooleanExpr([&]()
                                            { return v; });
    }
